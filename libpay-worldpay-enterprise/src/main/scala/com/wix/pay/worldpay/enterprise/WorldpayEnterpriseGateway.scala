...
import akka.http.scaladsl.client.TransformerPipelineSupport.WithTransformerConcatenation
...
/** A subclass of the `com.wix.pay.PaymentGateway`, for Worldpay gateway.
  *
  * @param endpointUrl
...
class WorldpayEnterpriseGateway(endpointUrl: String = Endpoints.production,
  ...
  import  HasXmlResponse.system
  import HasXmlResponse.materializer
  import HasXmlResponse.executionContext
  ...
  private def postRequest(merchant: WorldpayEnterpriseMerchant, requestXml: Node): HttpResponse = {
    ...
    val pipeline: WithTransformerConcatenation[HttpRequest, Future[HttpResponse]] = (
  ...
  override def authorize(merchantKey: String,
    ...
    withTry {
      ...
      val response = postRequest(
        merchant,
        WorldpayEnterpriseGatewayHelper.createAuthorizationRequest(
      ...
      response match {
        case WasAuthorizedSuccessfully(orderId) =>
          orderParser.stringify(WorldpayEnterpriseAuthorization(orderId, payment.currencyAmount.currency))

        case AuthorizationFailed(event, code, description) if event == "REFUSED" =>
          throw PaymentRejectedException(s"Error code: $code, Error message: $description")

        case AuthorizationFailed(event, code, description) if event == "ERROR" =>
          throw PaymentErrorException(s"Error code: $code, Error message: $description")

        case res =>
          val errorMessageFromWorldpay = Await.result(Unmarshal(res.entity).to[String], Duration.Inf)

          throw PaymentErrorException(s"Worldpay server returned ${res.status.intValue}: $errorMessageFromWorldpay")
      }
    }
  }
  ...
  override def voidAuthorization(merchantKey: String, authorizationKey: String): Try[String] = {
    ...
    withTry {
      ...
      val response = postRequest(
        merchant,
        WorldpayEnterpriseGatewayHelper.createVoidAuthorizationRequest(merchant.merchantCode, order.orderCode))

      response match {
        case WasCancelledSuccessfully(orderCode) =>
          orderParser.stringify(WorldpayEnterpriseAuthorization(orderCode, order.currency))

        case ModificationFailed(errorCode, errorDescription) =>
          throw PaymentErrorException(s"Error code: $errorCode, Error message: $errorDescription")
  ...
  override def capture(merchantKey: String, authorizationKey: String, amount: Double): Try[String] = {
    withTry {
      ...
      val response = postRequest(
        merchant,
        WorldpayEnterpriseGatewayHelper.createCaptureRequest(
          merchant.merchantCode,
          order.orderCode,
          CurrencyAmount(order.currency,
            amount)))

      response match {
        case WasCapturedSuccessfully(orderCode, currency) =>
          orderParser.stringify(WorldpayEnterpriseAuthorization(orderCode, currency))
...
object HasXmlResponse {
  implicit val system: ActorSystem = ActorSystem("akka-http-worldpay-gateway-system")
...
object WasAuthorizedSuccessfully {
  def unapply(response: HttpResponse): Option[String] = {
    response match {
      case HasXmlResponse(elem) if (elem \\ "lastEvent").text == "AUTHORISED" =>
        Some((elem \\ "orderStatus" \ "@orderCode").text)
...
object AuthorizationFailed {
  def unapply(response: HttpResponse): Option[(String, String, String)] = {
    response match {
      case HasXmlResponse(elem) if requestFailed(elem) =>
        Some((elem \\ "lastEvent").text,
          (elem \\ "ISO8583ReturnCode" \ "@code").text,
          (elem \\ "ISO8583ReturnCode" \ "@description").text)

      case HasXmlResponse(elem) if (elem \\ "error").nonEmpty =>
        Some("ERROR", (elem \\ "error" \ "@code").text, (elem \\ "error").text)
...
object WasCancelledSuccessfully {
  def unapply(response: HttpResponse): Option[String] = {
    response match {
      case HasXmlResponse(elem) if (elem \\ "cancelReceived").nonEmpty =>
        Some((elem \\ "cancelReceived" \ "@orderCode").text)
...
object ModificationFailed {
  def unapply(response: HttpResponse): Option[(String, String)] = {
    response match {
      case HasXmlResponse(elem) if (elem \\ "error").nonEmpty =>
        Some((elem \\ "error" \ "@code").text, (elem \\ "error").text)
...
object WorldpayEnterpriseGateway {
  private val WORLDPAY_DTD = xml.dtd.DocType(
    "paymentService",
    xml.dtd.PublicID(
